...
class FlowState:
    ...
    def summarize(self) -> Dict[str, float]:
        ...
        return {"size_mean": size_mean, "size_std": size_std, "interarrival_mean": ia_mean, "interarrival_std": ia_std, "spin_entropy": spin_ent}
...
class QuicTlsBehaviorIDS:
    ...
    def _parse_tls(self, data: bytes, flow: FlowState, client: bool):
        ...
        while pos + 5 <= len(data):
            ...
            pos += 2
            rec_len = struct.unpack("!H", data[pos:pos + 2])[0]; pos += 2
            if pos + rec_len > len(data):
                break
            rec_body = data[pos:pos + rec_len]; pos += rec_len
            if rec_type != 22 or len(rec_body) < 4:
                continue
            hs_type = rec_body[0]
            ...
            if hs_type == 1 and client:
                ...
                if idx >= len(ch):
                    continue
                sid_len = ch[idx]; idx += 1 + sid_len
                if idx + 2 > len(ch):
                    continue
                cs_len = struct.unpack("!H", ch[idx:idx + 2])[0]; idx += 2
                ciphers = ch[idx:idx + cs_len]; idx += cs_len
                if idx >= len(ch):
                    continue
                ...
                alpn_list: List[str] = []
                if idx + 2 <= len(ch):
                    ext_total = struct.unpack("!H", ch[idx:idx + 2])[0]; idx += 2
    ...
    def _evaluate_and_emit(self, key: Tuple, flow: FlowState, proto: str):
        ...
        if trigger:
            src_h = key[2]; dst_h = key[3]; sport = key[4]; dport = key[5]
            event = {
                "event_type": "alert",
                ...
                "alert": {
                    ...
                    "rules": {"version": self.rules.version},
                },
                "network": {
                    "proto": proto,
                    "src_hash": src_h,
                ...
                "privacy": {"pii_hashed": True, "hash_algo": "sha256", "salted": True},
                "retention": {"ttl_seconds": self.retention_seconds}
            }
