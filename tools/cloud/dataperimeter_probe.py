...
class Config:
    ...
    state_path: Optional[str] = None  # for simulated provider persistence
...
class AppendOnlyJSONLogger:
    def __init__(self, path: str):
        ...
        # Touch file to ensure it exists with correct permissions
        flags = os.O_CREAT
...
class Canary:
    ...
    def create(secret_key: str, ttl_seconds: int, scope: Dict[str, str], correlation_id: str) -> "Canary":
        ...
        payload = f"{cid}|{int(created)}|{int(expires)}|{json.dumps(scope, sort_keys=True)}|{correlation_id}".encode(
            "utf-8"
        )
        sig = base64.urlsafe_b64encode(hmac.new(secret_key.encode("utf-8"), payload, hashlib.sha256).digest()).decode(
            "utf-8"
        )
    ...
    def validate(self, secret_key: str) -> bool:
        payload = f"{self.id}|{int(self.created_at)}|{int(self.expires_at)}|{json.dumps(self.scope, sort_keys=True)}|{self.correlation_id}".encode(
            "utf-8"
        )
        expected = hmac.new(secret_key.encode("utf-8"), payload, hashlib.sha256).digest()
        try:
            got = base64.urlsafe_b64decode(self.signature.encode("utf-8"))
        except Exception:
            return False
    ...
    def to_metadata(self, label_key: str, label_value: str) -> Dict[str, str]:
        return {
            ...
            "canary_id": self.id,
            "canary_created_at": str(int(self.created_at)),
            "canary_expires_at": str(int(self.expires_at)),
            "canary_signature": self.signature,
...
class PolicyChecker:
    def __init__(self, config: Config):
        self.config = config
        self.opa_path = config.opa_policy_path

    def check(self, action: str, resource: Dict[str, Any]) -> Tuple[bool, str]:
        # Fallback Python guardrails policy: allow only writes to labeled canary resources, deny destructive ops
        # and ensure principals and regions are allowlisted.
        # If OPA policy path is provided and opa is installed, try to evaluate it.
        if self.opa_path:
            try:
                import shutil
                    ...
                    if proc.returncode == 0:
                        ...
                        for r in output.get("result", []):
                            for ex in r.get("expressions", []):
                                if isinstance(ex.get("value"), bool):
                                    allow = ex.get("value")
                        if allow:
                            return True, "allowed_by_opa"
        ...
        action_ok = True
        reason = "ok"
        # Writes and generates must target canary-labeled resources
        ...
        # Regions/accounts must be in allowlists
        region = resource.get("region")
        account = resource.get("account_id") or resource.get("project_id") or resource.get("subscription_id")
...
class SimulatedProvider(CloudProvider):
    ...
    def name(self) -> str:
        return self.state.get("provider", "simulated")
    ...
    def list_storage_buckets(self) -> List[Bucket]:
        ...
        for bn, bd in self.state.get("buckets", {}).items():
            res.append(
                Bucket(
                    name=bn,
                    region=bd.get("region", self.state.get("region")),
                    labels=bd.get("labels", {}),
                    account_id=account_id,
                    project_id=project_id,
                    subscription_id=subscription_id,
                    vpc_endpoint_required=bool(bd.get("vpc_endpoint_required", False)),
                    private_endpoint_ids=list(bd.get("private_endpoint_ids", []) or []),
                    vpc_sc_enabled=bool(bd.get("vpc_sc_enabled", False)),
                    allow_public_read=bool(bd.get("allow_public_read", False)),
                )
            )
        return res

    def _bucket_state(self, bucket: Bucket) -> Dict[str, Any]:
        return self.state["buckets"][bucket.name]

    def can_write_to_bucket(self, bucket: Bucket, config: Config) -> bool:
        labels = bucket.labels
        return labels.get(config.canary_label_key) == config.canary_label_value
    ...
    def create_canary_object(self, bucket: Bucket, canary: Canary, content: bytes) -> ObjectMeta:
        with self._lock:
            objid = f"canary-{canary.id}"
            bstate = self._bucket_state(bucket)
            objects = bstate.setdefault("objects", {})
            # Write-only to canary resources - ensure not overwriting production objects
            if objid in objects:
                # create with unique suffix to avoid overwrite
                objid = f"{objid}-{int(time.time())}"
            meta = {
                "created_at": canary.created_at,
                "labels": canary.to_metadata(label_key=self.state.get("canary_label_key", "dataperimeter"),
                                             label_value=self.state.get("canary_label_value", "canary")),
    ...
    def generate_presigned_url(self, bucket: Bucket, object_meta: ObjectMeta, expires_in: int) -> str:
        # Simulate by encoding details
        token = {
            "bucket": bucket.name,
            "object_id": object_meta.object_id,
            "region": bucket.region,
            "expires_at": int(utc_ts() + expires_in),
            "cid": object_meta.correlation_id,
        }
    ...
    def simulate_read_from_untrusted_network(self, bucket: Bucket, object_meta: ObjectMeta) -> bool:
        # Determine if a read from "untrusted" network would be allowed (i.e., leak)
        # Leak if: allow_public_read OR no VPC endpoint required AND no private endpoints AND VPC-SC disabled
        leak = bucket.allow_public_read or (
            (not bucket.vpc_endpoint_required) and (not bucket.private_endpoint_ids) and (not bucket.vpc_sc_enabled)
        )
        # Log an audit event if "attempted"
        with self._lock:
            ...
            self.state["audit_logs"].append(
                {
                    ...
                    "region": bucket.region,
                    "object_id": object_meta.object_id,
                    "correlation_id": object_meta.correlation_id,
                    "network": "untrusted",
                    "result": "Allowed" if leak else "Denied",
                }
            )
        return leak
    ...
    def access_presigned_url_from_region(self, url: str, access_region: str) -> Dict[str, Any]:
        ...
        b64 = url.split("?ps=")[1]
        token = json.loads(base64.urlsafe_b64decode(b64.encode("utf-8")).decode("utf-8"))
        cross_region = token["region"] != access_region
        # Simulate success regardless of region (as most services allow) but mark cross-region egress
        result = {
            "bucket": token["bucket"],
            "object_id": token["object_id"],
            "issued_region": token["region"],
            "access_region": access_region,
            "cross_region": cross_region,
            "correlation_id": token["cid"],
            "expires_at": token["expires_at"],
            "result": "Allowed" if utc_ts() <= token["expires_at"] else "Expired",
        }
        with self._lock:
            self.state.setdefault("audit_logs", [])
            self.state["audit_logs"].append(
    ...
    def get_audit_logs(self, correlation_id: str) -> List[Dict[str, Any]]:
        logs = self.state.get("audit_logs", [])
        return [e for e in logs if e.get("correlation_id") == correlation_id]

    def cleanup_expired_canaries(self, label_key: str, label_value: str, ttl: int, now_ts: float) -> int:
        count = 0
        with self._lock:
            for bname, bd in self.state.get("buckets", {}).items():
                objects = bd.get("objects", {})
                remove_keys = []
                for oid, meta in objects.items():
                    ...
                    if labels.get(label_key) == label_value:
                        ...
                        if now_ts - created_at > ttl:
                            remove_keys.append(oid)
                for oid in remove_keys:
    ...
    def subscription_id(self) -> Optional[str]:
        return self.state.get("subscription_id")

    def region(self) -> Optional[str]:
        return self.state.get("region")
...
class ProbeEngine:
    def __init__(self, config: Config, provider: CloudProvider, logger: AppendOnlyJSONLogger):
        ...
        self.logger = logger
        self.policy = PolicyChecker(config)
    ...
    def _enforce_allowlists(self, bucket: Bucket) -> Tuple[bool, str]:
        # Provider check also enforced in PolicyChecker, but double-check here
        ...
        pid = self.provider.account_id() or self.provider.project_id() or self.provider.subscription_id()
    ...
    def _check_rate_and_guard(self) -> None:
        if self.guard.should_pause():
            raise RuntimeError(f"Probing paused due to anomaly threshold reached: {self._paused_reason or 'excess anomalies'}")
    ...
    def probe_egress_read(self, bucket: Bucket) -> Dict[str, Any]:
        ...
        ok, reason = self._enforce_allowlists(bucket)
        if not ok:
        ...
        result: Dict[str, Any] = {
            ...
            "bucket": bucket.name,
            "region": bucket.region,
            "object_id": objmeta.object_id,
            "egress_leak_detected": allow_read,
            "details": {},
            "remediation": [],
        }

        if allow_read:
            # Leak path with remediation suggestions
            details = {
                "path": "untrusted_network_read",
                "vpc_endpoint_required": bucket.vpc_endpoint_required,
                "private_endpoint_ids": bucket.private_endpoint_ids,
                "vpc_sc_enabled": bucket.vpc_sc_enabled,
                "allow_public_read": bucket.allow_public_read,
            }
            remediation = []
            if bucket.allow_public_read:
                remediation.append("Disable public read access at bucket level and enable block-public-access.")
            if not bucket.vpc_endpoint_required:
                remediation.append("Enforce VPC/VNet/Private Endpoint access using explicit condition keys or service endpoints.")
            if not bucket.private_endpoint_ids:
                remediation.append("Scope access to approved PrivateLink/Private Endpoint service IDs.")
            if not bucket.vpc_sc_enabled:
                remediation.append("Enable VPC Service Controls or equivalent perimeter (GCP VPC-SC/Azure Private Endpoints).")
            result["details"] = details
            result["remediation"] = remediation
            self._log(correlation_id, {"event": "egress_leak_detected", "details": details, "severity": "high"})
            self.guard.record_anomaly()
            if self.guard.should_pause():
                self._paused_reason = "excessive egress leaks detected"
        else:
    ...
    def probe_presigned_cross_region(self, bucket: Bucket) -> Dict[str, Any]:
        ...
        # Access from cross-region (choose a region different than bucket.region)
        access_region = "eu-west-1" if bucket.region != "eu-west-1" else "us-east-1"
        if self.config.regions_allowed and access_region not in self.config.regions_allowed:
            # if not allowlisted, stick to current region to avoid violation
            access_region = bucket.region
        res = {}
        ...
        out = {
            ...
            "bucket": bucket.name,
            "object_id": objmeta.object_id,
            "issued_region": bucket.region,
            "access_region": res.get("access_region", access_region),
            "cross_region": res.get("cross_region", False),
            "presigned_url": url if self.config.dry_run else "<redacted>",
            "audit_events": logs,
        }
    ...
    def cleanup_canaries(self) -> Dict[str, Any]:
        self._check_rate_and_guard()
        # Cleanup only labeled canary resources and expired TTL
        cleaned = self.provider.cleanup_expired_canaries(
            self.config.canary_label_key, self.config.canary_label_value, self.config.canary_ttl_seconds, now_ts=utc_ts()
        )
        cid = str(uuid.uuid4())
        self._log(cid, {"event": "cleanup", "removed_count": cleaned})
        return {"correlation_id": cid, "removed_count": cleaned}


def load_config(path: Optional[str]) -> Config:
    # Minimal loader: support JSON only to stay dependency-free
    if not path:
        raise ValueError("config path is required")
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    return Config(
        providers_allowed=data.get("providers_allowed", ["simulated"]),
        accounts_allowed=data.get("accounts_allowed", []),
        projects_allowed=data.get("projects_allowed", []),
        subscriptions_allowed=data.get("subscriptions_allowed", []),
        regions_allowed=data.get("regions_allowed", []),
        rate_limit_per_minute=int(data.get("rate_limit_per_minute", 30)),
        anomaly_threshold=int(data.get("anomaly_threshold", 5)),
        anomaly_window_seconds=int(data.get("anomaly_window_seconds", 300)),
        log_path=data.get("log_path", "./dataperimeter_probe.log"),
        canary_label_key=data.get("canary_label_key", "dataperimeter"),
        canary_label_value=data.get("canary_label_value", "canary"),
        canary_ttl_seconds=int(data.get("canary_ttl_seconds", 3600)),
...
def load_simulated_state(path: Optional[str]) -> Dict[str, Any]:
    ...
    return {
        ...
        "region": "us-east-1",
        "buckets": {
            # Production bucket with missing constraints - but not labeled, won't be modified
            "prod-data": {
                "region": "us-east-1",
                "labels": {"env": "prod"},
                "vpc_endpoint_required": False,
            ...
            # Canary-enabled test bucket
            "dp-probe-canary-us-east-1": {
                "region": "us-east-1",
                "labels": {"dataperimeter": "canary", "ttl": "3600"},
                "vpc_endpoint_required": False,
                "private_endpoint_ids": [],
                "vpc_sc_enabled": False,
                "allow_public_read": False,
            ...
            "dp-probe-strict": {
                ...
                "private_endpoint_ids": ["vpce-12345"],
                "vpc_sc_enabled": True,
                "allow_public_read": False,
                "objects": {},
            },
        ...
        "audit_logs": [],
        "canary_label_key": "dataperimeter",
        "canary_label_value": "canary",
    }
...
def find_canary_buckets(provider: CloudProvider, config: Config) -> List[Bucket]:
    buckets = provider.list_storage_buckets()
    res = []
    for b in buckets:
        if b.labels.get(config.canary_label_key) == config.canary_label_value:
            res.append(b)
    return res
...
def cli():
    ...
    p_probe = sub.add_parser("probe-egress", help="Probe egress on canary buckets")
    p_probe.add_argument("--bucket", help="Specific bucket to probe")

    p_pre = sub.add_parser("probe-presigned", help="Probe presigned URL cross-region access")
    p_pre.add_argument("--bucket", help="Specific bucket to probe")

    sub.add_parser("cleanup", help="Cleanup expired canaries")

    args = parser.parse_args()
    cfg = load_config(args.config)
    state = load_simulated_state(cfg.state_path)
    provider = SimulatedProvider(state)
    logger = AppendOnlyJSONLogger(cfg.log_path)
    engine = ProbeEngine(cfg, provider, logger)

    if args.cmd == "init-state":
        if cfg.state_path and not os.path.exists(cfg.state_path):
            save_simulated_state(cfg.state_path, state)
            print(f"Initialized simulated state at {cfg.state_path}")
        else:
    ...
    if args.cmd == "status":
        buckets = provider.list_storage_buckets()
        canary_buckets = find_canary_buckets(provider, cfg)
        out = {
            ...
            "canary_buckets": [b.name for b in canary_buckets],
        }
    ...
    if args.cmd == "probe-egress":
        results = []
        targets = find_canary_buckets(provider, cfg)
        if args.bucket:
            targets = [b for b in targets if b.name == args.bucket]
        for b in targets:
            try:
                r = engine.probe_egress_read(b)
                results.append(r)
            except Exception as e:
    ...
    if args.cmd == "probe-presigned":
        ...
        for b in targets:
            try:
                r = engine.probe_presigned_cross_region(b)
                results.append(r)
            except Exception as e:
                results.append({"bucket": b.name, "error": str(e)})
        print(json.dumps(results, indent=2))
